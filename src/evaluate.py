...
def main():
    ...
    train_ds, val_ds, test_ds, train_loader, val_loader, test_loader = make_loaders(
        data_dir=data_dir, mean=mean, std=std, batch_size=int(cfg["train"]["batch_size"]), num_workers=int(cfg["data"].get("num_workers", 2)), balanced=False
    )
    num_classes = len(train_ds.classes)

    pretrained = bool(cfg["model"].get("pretrained", False)) or bool(args.use_pretrained)
    if not pretrained:
        model = build_from_scratch(num_classes=num_classes, dropout=float(cfg["model"].get("dropout", 0.3)), hidden_dim=int(cfg["model"].get("hidden_dim", 512)))
    else:
        model = build_densenet121(num_classes=num_classes, pretrained=True, fine_tune=args.fine_tune, dropout=float(cfg["model"].get("dropout", 0.3)), hidden_dim=int(cfg["model"].get("hidden_dim", 512)))
    ...
    out = {"val_loss": val_loss, "val_acc": val_acc, "test_loss": test_loss, "test_acc": test_acc}
